...
> **중요 변경**: 환경 구축에 충분한 시간 확보!
> 모든 학생이 동일한 환경! 문제 있으면 즉시 해결!
...
### 1교시: Day 6 복습 + FastAPI 시작 ✨

- **복습 퀴즈 (10min)**: JSON, HTTP, REST API, Postman
- **핵심 복습 (15min)**: 
  - JSON 파일 읽기/쓰기 복습
  - HTTP 메서드 복습
  - 간단한 Q&A
...
### 2교시: Pydantic + Request/Response 통합 ⭐⭐⭐ ✨ 개선!
...
- **복습 퀴즈 (10min)**: SQL, SQLite, CRUD
- **🟢 기초 Problem (15min)**: 간단한 메모 API (DB 포함)
- **🟡 응용 Problem (15min)**: 사용자 관리 시스템
- **🔴 도전 Problem (10min)**: 검색 기능 추가
...
3. **Pydantic Validator**: 커스텀 검증 (추가!)
4. **경로/쿼리 매개변수**: API 엔드포인트 설계
5. **CRUD 작업**: 데이터베이스 연동
6. **REST API 설계**: RESTful 원칙 적용
7. **API 버전 관리**: v1, v2 설계 (추가!)
8. **에러 처리**: HTTPException, 상태 코드
9. **Postman**: API 테스팅
...
1. ✅ **1교시 복습 간소화**: 10분 퀴즈 + 15분 복습 + 25분 진도
2. ✅ **Day 7-7교시**: Response 제거 → 종합 실습 (경로+쿼리)
3. ✅ **Day 8-2교시**: Pydantic + Request/Response 통합
4. ✅ **Day 8-4교시**: Pydantic Validator 추가 (20분)
5. ✅ **Day 10-2교시**: API 버전 관리 추가 (20min)
6. ✅ **Day 10-6교시**: 프로젝트 설계만 집중
7. ✅ **Day 10-8교시**: 구현 시간 확대 (30min)
8. ✅ 프로덕션 레벨 기능 강화
9. ✅ **총 125분 시간 확보** (응용 과정에서)
