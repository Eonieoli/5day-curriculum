...
### 2교시: FastAPI 첫 API + requests 기초 ⭐⭐⭐ ✨ 변경!

- **FastAPI 첫 API (30min)**: ✨ 50분 → 30분
  - **Basic 완성 (15min)**:
    - Hello World API 완성
    - uvicorn 실행
    - 브라우저 확인
...
- **requests 라이브러리 기초 (20min)**: ✨ NEW!
  - **개념 (5min)**:
    - requests란? Python으로 HTTP 요청
    - 외부 API 호출의 핵심
  ...
  - **Basic (10min)**:
    ...
    ```
  - **Practice (3min)**:
    - 공개 API 호출 (JSONPlaceholder)
    - 응답 데이터 출력
  - **Exercise (2min)**:
    - GitHub API로 사용자 정보 가져오기

> **중요 추가**: Python으로 외부 API 호출!
> FastAPI만 만들지 말고, 외부 API도 호출할 줄 알아야 함!
...
- **복습 퀴즈 (10min)**: FastAPI 기본, 경로/쿼리 매개변수, requests
- **🟢 기초 Problem (15min)**: 간단한 도서관 API (책 조회)
- **🟡 응용 Problem (15min)**: 쇼핑몰 상품 API (경로+쿼리)
- **🔴 도전 Problem (10min)**: requests로 외부 API 호출 + 가공
...
- **복습 퀴즈 (10min)**: FastAPI 기본, 매개변수, requests
- **핵심 복습 (15min)**: 
  - 경로/쿼리 매개변수 복습
  - 자동 문서 확인
  - 간단한 Q&A
...
### 7교시: FastAPI + SQLite 통합 + JOIN 기초 ⭐⭐⭐ ✨ 변경!
...
1. ✅ **Day 6-3교시**: 웹 동작 원리 축소 (50분 → 30분)
2. ✅ **Day 7-2교시**: requests 라이브러리 추가 (20분)
3. ✅ **Day 9-7교시**: SQL JOIN 기초 추가 (25분)
4. ✅ v7.0 모든 개선사항 유지
5. ✅ **총 45분 시간 확보 및 활용** (응용 과정에서)
